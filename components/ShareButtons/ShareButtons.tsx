...
export default function ShareButtons({ title, url, className = '' }: { title?: string; url?: string; className?: string }) {
  ...
  const shareOnTwitter = () => {
    ...
    window.open(`https://twitter.com/intent/tweet?text=${text}&url=${link}`, '_blank', 'width=600,height=400,noopener,noreferrer');
  };

  const shareOnLinkedIn = () => {
    const link = encodeURIComponent(shareUrl);
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${link}`, '_blank', 'width=600,height=400,noopener,noreferrer');
  };

  const shareOnWhatsApp = () => {
    const text = encodeURIComponent(`${shareTitle} ${shareUrl}`);
    window.open(`https://api.whatsapp.com/send?text=${text}`, '_blank', 'width=600,height=400,noopener,noreferrer');
  };

  const shareOnTelegram = () => {
    const text = encodeURIComponent(shareTitle);
    const link = encodeURIComponent(shareUrl);
    window.open(`https://t.me/share/url?url=${link}&text=${text}`, '_blank', 'width=600,height=400,noopener,noreferrer');
  };
    ...
    <section className={`py-12 border-t border-white/5 ${className}`}>
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-6">
          <h3 className="text-xl font-bold text-white mb-2">شارك هذه الصفحة</h3>
          <p className="text-text-secondary text-sm">انشر المحتوى مع شبكتك</p>
        </div>
