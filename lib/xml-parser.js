...
// Deduplicate article content by removing repeated h2 sections
function deduplicateArticleContent(content) {
  if (!content) return content;

  // Split by h2 tags, keeping the delimiter
  const parts = content.split(/(<h2[^>]*>.*?<\/h2>)/i);

  // If no h2 tags, return as is
  if (parts.length <= 1) return content;

  const sections = [];
  const seenHeadings = new Set();

  // parts[0] is content before first h2 (if any)
  if (parts[0].trim()) {
    sections.push(parts[0]);
  }

  // Process each h2 + following content pair
  for (let i = 1; i < parts.length; i += 2) {
    const heading = parts[i];
    const body = parts[i + 1] || '';

    // Extract plain heading text for comparison
    const headingText = heading.replace(/<[^>]+>/g, '').trim().toLowerCase();

    // Skip if we've already seen this heading (keep first occurrence only)
  ...
  for (const xmlFile of xmlFiles) {
    ...
    try {
      ...
      for (const item of itemsArray) {
        ...
        let content = cleanArabicText(fixEncoding(item['content:encoded'] || ''));
        // Deduplicate repeated h2 sections
        content = deduplicateArticleContent(content);
